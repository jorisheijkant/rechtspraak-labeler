...
This project uses Playwright to scrape a website containing court documents. It then uses a LLM of choice to label these and export them as a spreadsheet.

## Key Directories

- `classes/` - the classes used to label the documents
- `functions/` - most python code lives here 
- `data/` - where possible data can be stored (not committed)
- `variables/` - where variables like the used prompt are stored (N.B.: API keys are saved in the `.env`)
